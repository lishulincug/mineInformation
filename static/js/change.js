$(document).ready(function(){
	...
	switch(applyId){
		...
		case '104':
			...
			$('.tableHead').html("石油天然气开采企业安全生产许可证&nbsp;&nbsp;变更申请书");
		break;
		case '105':
			companytype = '尾矿库';
		break;
			...
			$("#choose").click(function(){
				if ($('#searchApplyCompany input:checked').length == 1){
					...
					var f = function(b){
						...
						a[3] = b.data[0].unit[8];//法定代表人
						a[4] = b.data[0].unit[6];//从业人数
						a[5] = b.data[0].unit[11]//安全负责人
						a[6] = b.data[0].unit[7];//专职安全人员数
						a[7] = b.data[0].unit[12];//办公电话
						a[8] = b.data[0].unit[14];//工商注册号
						a[9] = b.oldPermit;
		...
		function checkout(){
			if (($('#applyNumber').val()
				...
				var e = {
					...
					// 	['1','dsfdf','c','d'],['2','b','c','d']	
					// ]})
					pcdata('post', 'ChangeTransactionServlet', e, 'json', false, f);
					// userlog(a);
			};
		}
		checkout();
